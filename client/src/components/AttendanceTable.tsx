      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead>
          <tr className="bg-gray-50">
            ...
            <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider sticky right-0 bg-gray-50 border-l border-gray-200 z-10">
              {canEdit ? '操作' : '狀態'}
            </th>
          </tr>
        </thead>
        <tbody className="bg-white divide-y divide-gray-200">
          {data.length === 0 ? (
            ...
            data.map((record, index) => {
              ...
              // sticky 欄需要明確的背景色才不會透視下方內容
              const stickyBg = isHolidayRecord
                ? 'bg-gray-100'
              ...
              return (
                <tr key={record.id} className={rowClassName}>
                  ...
                  <td className={`px-6 py-4 whitespace-nowrap text-center sticky right-0 z-10 border-l border-gray-200 ${stickyBg}`}>
                    {isNoClockType ? (
