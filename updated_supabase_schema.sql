-- Safe, idempotent Supabase/PostgreSQL upgrade bootstrap for the current runtime schema.
-- This file intentionally avoids any destructive reset behavior:
--   - no DROP TABLE
--   - no seeded admin PIN
--   - no sample employees / holidays / salary data
--   - no legacy users/password table
--
-- Use when an operator needs a SQL Editor path to reconcile an existing database with
-- the current application schema. For the normal repo-managed flow, prefer:
--   npm run db:push
...
-- Upgrade legacy/partial installations in place.
DO $$
BEGIN
...
COMMENT ON TABLE settings IS
  'Operator must provide DEFAULT_ADMIN_PIN at first boot, or let the server generate a random bootstrap PIN. This SQL intentionally seeds no admin PIN.';

-- Drop known legacy permissive policies, then enforce default-deny public-schema posture.
DO $$
DECLARE
  table_name TEXT;
  policy_name TEXT;
...
-- Operator notes:
--   1. This script does not reset live data.
--   2. This script does not insert a settings row or any default admin PIN.
--   3. If your existing settings row lacks admin_pin, backfill it before first admin login,
--      or remove the broken row and let runtime bootstrap a fresh one from DEFAULT_ADMIN_PIN.
